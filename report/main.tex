...
\begin{enumerate}
    \item \textbf{Encoding.} As mentioned, the encoding we chose was to represent texts as a sequence of part of speech tags. %TODO vocab size 
    \item \textbf{Embedding Layer} %TODO Harry
    \item \textbf{LSTM}
    \item \textbf{Linear}
    \item \textbf{Softmax}
\end{enumerate}
...
\paragraph{Encoding}
In addition to the sequence of part of speech tags, for the generation task, we decided to directly encode the punctuation instead of having them all transformed to the same tag \textit{PUNCT}. Furthermore, we introduce two special characters '$<$', and '$>$' which are used to signify the beginning and the end of a text. Apart from these changes, the model architecure is equivalent to that in the previous section.

\subsection{Training}

\subsection{Sampling}
Let $G$ be the trained model, $v$ be the vocabulary, and $x$ be a document. It is an approximation such that \[G(x_1,x_2,\dots,x_{t-1}) = \begin{bmatrix}
...
Where $x_t$ is the $t^{th}$ part of speech tag in the document $x$. %
