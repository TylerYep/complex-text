...
First, we will compare the differences between texts of different labels to see if there are certain features that are indicative of certain labels, such as the lengths of sentences or number of unique words. We will use the results of this analysis to guide our feature selection and text representation. 

The text encoding schemes we will experiment with are \textit{bag of words}, \textit{word counts}, and \textit{Tf-Idf}. We will test the features using a simple softmax regression, and then combine them to measure their effectiveness.
