...
Empirically, we found that performance of models were not very sensitive to the $min_df$ and $max_df$ parameters. However, the $binary$ option either substantially increased or decreased accuracy. Thus we set $min_{df}$, $max_{df} = 5, 80\%$, and try both binary and not-binary. 
...
We also appended counts of the parts of speech in each of our documents. Using spaCy, we parsed each of the words in our documents and categorized them
We also added the average sentence length and basic features such as the number of sentences and number of words. (Add something about normalizing for longer texts)
